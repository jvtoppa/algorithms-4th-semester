...
int teste_1() {
    ...
    res -> first = 0;
    res -> size = 8;
    if(!df_pop(res)) return 0;
    if(res -> first != 0) return 0;
    if(res -> size != 7) return 0;
...
int teste_3() {
    // Simple pop with resizing
    float cur_data[8] = {1.0, 2.0, 3.0, 5.0, 8.0, 13.0, 21.0, 34.0};
    dequef *res = df_alloc(8, 2.0);
    for(int i = 0; i < 3; i++) res -> data[i] = cur_data[i];
    res -> first = 0;
    res -> size = 3;
    res -> mincap = 4;
    if(!df_pop(res)) return 0;
    if(res -> size != 2) return 0;
    if(res -> cap != 4) return 0;
    if(res -> mincap != 4) return 0;
    if(!compare_floats(res -> factor, 2.0)) return 0;
    for(int i = 0; i < 2; i++) {
        if(!compare_floats(cur_data[i], df_get(res, i))) return 0;
    }
...
int teste_4() {
    // Simple pop with first in the middle with resizing
    float cur_data[8] = {1.0, 2.0, 3.0, 5.0, 8.0, 13.0, 21.0, 34.0};
    dequef *res = df_alloc(8, 2.0);
